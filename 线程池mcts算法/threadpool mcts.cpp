    ...
    void workerThread() {
        for (;;) {
...

            {
                ...
                while (this->stop == false && this->tasks.empty()) {
                    this->condition.wait(lock); // 条件不满足，释放锁并等待唤醒
                }
...
// MCTS单次模拟函数
void simulateWorker(treeNode* rootNode, int* count) {
...
int main()
{
    ...
    while (clock() - start < timeout) {
        pool.enqueue(simulateWorker, &rootNode, &count);
    }
