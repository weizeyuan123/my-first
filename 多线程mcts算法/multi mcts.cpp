...
public:
    ...
    {
        lock_guard<mutex> lock(childMutex); // 加锁保护子节点相关操作
        ...
        for (int i = 0; i < childrenCount; ++i)
        {
            if (children[i] == nullptr) {
                cerr << "Error: children[" << i << "] is nullptr" << endl;
                continue;
            }
...
// MCTS多线程模拟函数
void simulateWorker(treeNode* rootNode, int* count, clock_t start, int timeout) {
...
int main()
{
    ...
    // 启动多线程模拟
    int threadNum = 12;// thread::hardware_concurrency();
   // if (threadNum == 0) threadNum = 4; // 默认4线程

    vector<thread> threads;
    for (int i = 0; i < threadNum; i++) {
        threads.emplace_back(simulateWorker, &rootNode, &count, start, timeout);
        //threads.push_back(thread(simulateWorker, &rootNode, &count, start, timeout));
    }
    ...
    sprintf(buffer, "搜索节点数:%d,平均value:%.5f,用时：%.3f 核数为%d", count, (((double)(rootNode.children[bestChildren[random]]->value)) / ((double)rootNode.children[bestChildren[random]]->n) + 1.0) * 0.5, (double)(clock() - start) / 1000, threadNum);
